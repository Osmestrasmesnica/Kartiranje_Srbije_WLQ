...
import proba123 from './PhD.json' assert { type: 'json' };
...
//!KAKO DA DAM VREDNOST OVOM DATA IZNAD STO SAM IMPORTOVAO
  ...
  const podaciUTM = proba123.filter(podatak => (
    podatak.PROBA10x10 !== undefined && podatak.PROBA10x10 !== "Neprecizan podatak" /*&& podatak.PROBA10x10.length == 4*/
  ));
  ...
  for (let i=0; i < kockice.length; i++) {
    const proba10 = podaciUTM[i].PROBA10x10;
    const probaTip = podaciUTM[i].Tip_podatka;   
    const zaUbacivanje = kockice[i];
